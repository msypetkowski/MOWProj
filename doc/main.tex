...
\texttt{P\_G2} and
\texttt{P\_G3}.
Attribute \texttt{paid} also concerns particular course, so we create attributes
...
The results of this clustering may be considered commonsensial.
...
particurarly stable or standing out in any other way.
...
\end{itemize}
Tree with lowest mean error is shown in figure \ref{fig:single}.
...
particurarly stable or stand out in any other way.
...
(mean error around 33\% for single trees, and 30\% for ramdom forests).
...
Since random forest is widely used algorihm with many successes, we suspect
that it is hard or impossible to find significantly better
sudent alcohol consumption classification rule
with our dataset.
